...
App.handleLocalCsvParsed
  ...
  • lines = localCsvPointsToPlanLines(points)
      └─ buildRoadMarkingPreviewPoints (straights + arcs, open path)
  • setSelectedLineId("local-csv-path")
...
- One open plan line: id **`local-csv-path`**, layer **`marking`**
- Entity type `LWPOLYLINE`, `geometry.closed: false`, `geometry.road_marking: true`
...
2. **`ensureOpenPath`** — if first≈last, drop last → **never a closed polygon ring**
3. **`filletSharpCorners`** — turns ≳ 12° get circular fillets (when legs are long enough)
4. **`segmentAndTessellate`** — greedy longest **line** or **circular arc** fits (~8 cm tolerance), then densify (~0.35 m samples)
5. Open-path check again

Defaults (tunable via options):

| Parameter | Default | Role |
|-----------|---------|------|
| `fitToleranceM` | 0.08 m | Line/arc residual |
| `sharpCornerDeg` | 12° | Fillet threshold |
| `filletRadiusFraction` | 0.4 | Of shorter leg |
| `maxFilletRadiusM` | 8 m | Fillet cap |
| `sampleSpacingM` | 0.35 m | Preview density |
| `minArcPoints` | 4 | Min for arc fit |
...
|--------|--------|
| Test files | 2 |
| Tests | 21 (parse, pins, open path, fillets, arcs, plan tags) |
...
| Line | Single continuous open stroke (not N separate sticks) |
...
| Arc model | Circular only; residual up to ~fitToleranceM |
...
**Fields CSV is parsed fully on-device, drawn as an open road-marking path (straights + filleted curves, no polygons), with gold pins at exact survey lat/lon (sampled), without the blue dense-vertex selection clutter.**
