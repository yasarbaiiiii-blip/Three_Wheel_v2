/**
 * Upload → plan → stage chain for the Fields CSV flow.
 *
 * Mirrors the DXF flow's `pathApi.loadToController`, minus the two steps that only mean
 * something for a DXF: entity order and per-entity spray overrides are sidecar state keyed
 * to CAD entity ids, and a survey CSV has none.
 *
 ...
 *
 * When `CSV_PLANNER === "app"`, use {@link planAndStageAppTrajectory} instead of the
 * survey-file upload path (Phase 4 / 8).
 */
...
import type { SurveyGroundTruthPoint, TrajectoryRun } from "./csvTrajectory";
import {
