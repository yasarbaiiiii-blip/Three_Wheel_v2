/**
 * CSV geometry / parse readiness for Path Order and Send-to-Rover.
 *
...
import type { CsvPathOrderEntry } from "./csvPathOrder";
import { resolveOrderedPaintedLines, selectMarkPlanLines } from "./csvPathOrder";
...
export function isCriticalParseWarning(warning: string): boolean {
  return /projected|jumbled|swapped|Headerless|do not paint|confirm this is correct|local metres|cluster median|outlier|Null Island|near 0°|origin set to cluster/i.test(
    warning
  );
}
...
export function evaluateCsvSendReadiness(opts: {
  ...
  for (const m of nonPaintablePainted) {
    const detail =
      m.warnings[0] != null
        ? `${m.label}: ${m.warnings[0]}`
        : `${m.label}: geometry marked non-paintable`;
    needsAck.push(detail);
  }
